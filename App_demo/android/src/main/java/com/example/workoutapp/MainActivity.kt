...
class MainActivity : AppCompatActivity() {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == ADD_RECIPE_REQUEST_CODE && resultCode == Activity.RESULT_OK) {
            data?.let { intent ->
                val recipeTitle = intent.getStringExtra("TODO_TITLE")
                ...
                if (recipeTitle != null && recipeIngredients != null && recipeInstructions != null) {
                    val recipe = Recipe(recipeTitle,
                        recipeIngredients.toString(), recipeInstructions.toString(), recipeDifficulty,recipeTime)
